        ...
        if (self.gripper_state['pressure_reached'] == True and \
            ...
            # If its not, try to attach again
                ...
                await self.send_feedback(
                    'Actual desired position not reached. Attaching again...'
                )
                self.state = 'attaching'
...
###############################################################################
    ...
    async def gripper_feedback_cb(self, gripper_result):
        ...
        ## function updates local list of parameters which define the gripper 
        ## state
        ...
        if abs(gripper_result['final_position'] - \
...
###############################################################################
    ...
    async def avoid_obstacle(self):
        #### function activated if object or humen detected suddenly between 
        ### the robot and cart the function stops gary from moving, 
        ### alert with beeping sound and apply sleep method for 
        ### OBSTACLE_SLEEPING_TIME seconds.
        ### after MAX_OBSTACLE_INDEX times of identification, 
        ### the function abort the skill
    ...
    async def   major_angle_correction (self):
        ...
        while abs(self.angle) > MIN_STARTING_ANGLE and index < MAX_ANGLE_CORRECTION_ATTEMPTS:
            ...
            self.log.info(f'major angle correction. CP1')
            await self._timeout_verification()
            self.log.info(f'major angle correction. CP2')
            await self.adjust_angle()
            self.log.info(f'major angle correction. CP3')
            await self.read_srf_values()
            self.log.info(f'major angle correction. CP4')
            await self.calculate_distance_parameters()
            self.log.info(f'major angle correction. CP5')
            await self._cart_max_distance_verification()
            self.log.info(f'major angle correction. CP6')
            index+=1
            self.log.info(f'major angle correction. CP7')
    ...
    async def read_srf_values(self):
        ...
        while(True):
            ...
            if(math.isnan(srf_right)):
                ...
                # self.middle_srf = \
                # FILTER_WEIGHT * self.dr + (1-FILTER_WEIGHT) * srf_middle  
                self.dr = \
                    FILTER_WEIGHT * self.dr + (1-FILTER_WEIGHT) * srf_right
                self.dl = \
                    FILTER_WEIGHT * self.dl + (1-FILTER_WEIGHT) * srf_left
...
###############################################################################
    ...
    async def attach(self):
        ...
        if (is_moving):
            self.log.info("attach: CP1")
            await self.motion.cancel_motion()
            self.log.info("attach: CP2")
        try:

            self.log.info("attach: CP3")
